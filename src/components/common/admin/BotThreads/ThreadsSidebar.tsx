...
export default function ThreadsSidebar({ currThread, setcurrThread, threadSidebarOpen, botData }: Props) {
  const { id } = useParams()
                ...
                <div className='space-y-1'>
                  <div className='flex items-center gap-x-1'>
                    <p
                      key={thread._id}
                      className={cn(
                        'w-28 rounded-lg text-sm font-medium whitespace-nowrap overflow-hidden overflow-ellipsis'
                      )}
                    >
